...
import { sampleReservation } from "../data/mockData";
...
export default function MyBookingPage({ reservation, onCancel }) {
  ...
  const [found, setFound] = useState(Boolean(reservation));
  const [error, setError] = useState("");
  const booking = reservation ?? sampleReservation;
  ...
  return (
    <section className="mx-auto max-w-5xl px-5 py-12 lg:px-8">
      <header className="max-w-2xl">
        ...
        <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Find your reservation.</h1>
        <p className="mt-4 leading-7 text-forest-900/60">Use your reservation number and booking email to view your stay details or request a cancellation.</p>
      </header>
      ...
      <form onSubmit={searchBooking} className="mt-8 grid gap-4 rounded-3xl border border-forest-900/10 bg-white p-6 shadow-soft md:grid-cols-[1fr_1fr_auto] md:items-end" noValidate>
        <FormField id="reservation-reference" label="Reservation number">
        ...
        <article className="mt-8 rounded-3xl border border-forest-900/10 bg-white p-6 shadow-soft" aria-labelledby="booking-found-title">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-forest-900/55">Booked online · Specific room assigned later</p>
            </div>
          ...
          <aside className="mt-6 flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            ...
            Cancellation eligibility and refund amount must be confirmed by the backend according to the booking policy.
          </aside>
          ...
          <button type="button" onClick={() => onCancel(booking)} className="mt-6 inline-flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 font-semibold text-red-700">
            <XCircle size={17} aria-hidden="true" /> Request cancellation
          </button>
        </article>
      )}
    </section>
